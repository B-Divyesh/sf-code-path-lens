...
<title>{title}</title>
<style>
:root{{--paper:#e8ddc2;--ink:#172033;--muted:#535b68;--rule:#384359;--call:#1859a9;--entry:#a83220;--type:#35622d;--boundary:#7a5100;--unresolved:#8c2c3a;--focus:#995f00;--panel:#f7f0de}}
...
<script>
const graph=JSON.parse(document.querySelector('#lens-data').textContent);const nodes=document.querySelector('#nodes'),filter=document.querySelector('#filter'),empty=document.querySelector('#empty'),svg=document.createElementNS('http://www.w3.org/2000/svg','svg'),buttonById=new Map();let buttons=[];svg.classList.add('links');svg.setAttribute('aria-hidden','true');nodes.append(svg);
...
graph.nodes.forEach((node,i)=>{{const b=document.createElement('button');b.type='button';b.className='node';b.dataset.kind=node.kind;b.style.setProperty('--i',i);b.setAttribute('aria-pressed','false');b.innerHTML=`<strong></strong><small></small>`;b.querySelector('strong').textContent=node.label;b.querySelector('small').textContent=kindLabel(node.kind)+(node.location?` · ${{node.location.path}}:${{node.location.line}}`:'');b.addEventListener('click',()=>select(node,b));b.addEventListener('keydown',e=>{{if(!['ArrowRight','ArrowDown','ArrowLeft','ArrowUp'].includes(e.key))return;e.preventDefault();const shown=buttons.filter(x=>!x.hidden),at=shown.indexOf(b),step=['ArrowRight','ArrowDown'].includes(e.key)?1:-1;shown[(at+step+shown.length)%shown.length]?.focus();}});nodes.append(b);buttons.push(b);buttonById.set(node.id,b);}});if(graph.nodes[0])select(graph.nodes[0],buttons[0]);
function drawLinks(){{svg.replaceChildren();svg.setAttribute('viewBox',`0 0 ${{nodes.scrollWidth}} ${{nodes.scrollHeight}}`);graph.edges.forEach(edge=>{{const from=buttonById.get(edge.from),to=buttonById.get(edge.to);if(!from||!to||from.hidden||to.hidden)return;const line=document.createElementNS('http://www.w3.org/2000/svg','line');line.setAttribute('x1',from.offsetLeft+from.offsetWidth/2);line.setAttribute('y1',from.offsetTop+from.offsetHeight/2);line.setAttribute('x2',to.offsetLeft+to.offsetWidth/2);line.setAttribute('y2',to.offsetTop+to.offsetHeight/2);line.dataset.kind=edge.kind;svg.append(line);}});}}requestAnimationFrame(drawLinks);window.addEventListener('resize',drawLinks);
...
function applyFilter(){{const q=filter.value.trim().toLowerCase();let shown=0;graph.nodes.forEach((node,i)=>{{const visible=!q||`${{node.label}} ${{node.kind}} ${{node.evidence}} ${{node.location?.path||''}}`.toLowerCase().includes(q);buttons[i].hidden=!visible;if(visible)shown++;}});empty.hidden=shown>0;requestAnimationFrame(drawLinks);}}filter.addEventListener('input',applyFilter);document.addEventListener('keydown',e=>{{if(e.key==='/'&&document.activeElement!==filter){{e.preventDefault();filter.focus();}}if(e.key==='Escape'&&document.activeElement===filter){{filter.value='';applyFilter();filter.blur();}}}});
